		...
		for {
			...
			submitGeminiUsage := func(partial *service.ForwardResult) {
				...
				if partial.ReasoningEffort == nil {
					partial.ReasoningEffort = service.NormalizeClaudeOutputEffort(parsedReq.OutputEffort)
				}
				if partial.ReasoningEffort == nil && parsedReq.ThinkingEnabled {
...
func (h *GatewayHandler) Models(c *gin.Context) {
	...
	if responsePlatform == service.PlatformOpenAI && apiKey != nil && apiKey.Group != nil &&
	...
	if apiKey != nil && apiKey.Group != nil && apiKey.Group.ModelAllowlistEnabled() {
		writeAllowlistedModelsList(c, responsePlatform, availableModels)
		return
	}
...
func (h *GatewayHandler) modelsForGroupID(ctx context.Context, groupID *int64, group *service.Group, forcedPlatform string, injectDefaults bool) []string {
	...
	if group != nil && group.ModelAllowlistEnabled() {
		...
		return group.ModelAllowlist.FilterForListing(modelListingSource(platform, models, fallbackModels))
	}
	if len(models) > 0 {
		return models
	}
