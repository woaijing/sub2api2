...
func initializeApplication(buildInfo handler.BuildInfo) (*Application, error) {
	...
	apiKeyService := service.ProvideAPIKeyService(apiKeyRepository, userRepository, groupRepository, userSubscriptionRepository, userGroupRateRepository, apiKeyCache, configConfig, billingCacheService, concurrencyService, deferredService)
	...
	userHandler.SetCFAllowlistService(service.NewCFAllowlistService(service.NewCFAllowlistRepository(db), userService, configConfig))
	...
	balancePreauthorizationRecoveryWorker := service.NewBalancePreauthorizationRecoveryWorker(usageBillingRepository, balancePreauthorizationService)
	usageBalanceSettlementRepository := repository.NewUsageBalanceSettlementRepository(db)
	usageBalanceSettlementWorker := service.NewUsageBalanceSettlementWorker(usageBalanceSettlementRepository)
