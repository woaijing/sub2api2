<template>
  <component :is="isPopup ? 'div' : AppLayout" :class="isPopup ? 'min-h-screen bg-gray-50 dark:bg-dark-900' : ''">
    <div class="mx-auto max-w-lg space-y-6 py-8" :class="isPopup ? 'px-4' : 'console-provider-payment'">
      <header v-if="!isPopup" class="console-account-heading">
        <span class="console-account-index">STRIPE</span>
      ...
      <div v-else-if="initError" class="card p-8 text-center">
        ...
        <h2 class="text-lg font-semibold text-gray-900 dark:text-white">{{ t('payment.stripeLoadFailed') }}</h2>
      ...
      <template v-else>
        ...
        <div v-if="order" class="card overflow-hidden">
          <div class="console-provider-amount bg-gradient-to-br from-[#635bff] to-[#4f46e5] px-6 py-6 text-center">
