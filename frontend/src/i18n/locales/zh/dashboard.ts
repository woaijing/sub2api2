export default {
  dashboard: {
    ...
    infiniteCanvas: '无限画布',
    infiniteCanvasDesc: '侧边栏内直接作图、视频和对话',
    viewUsage: '查看使用记录',
  ...
  keys: {
    ...
    selectGroup: '选择分组',
    smartRouting: '智能路由',
    smartRoutingHint: '可添加多个分组并调整候选顺序，可跨平台。请求会先跳过账号模型目录里没有该模型的分组，再按列表顺序尝试；当前组没有可调度账号时才试下一组。客户端探针、空模型不会打到对不上的分组。',
    smartRoutingAdd: '添加分组',
    smartRoutingRoute: '路由',
    smartRoutingMax: '不限制分组数量',
    smartRoutingMore: '+{count}',
    smartRoutingChangeInEditor: '智能路由密钥请点编辑修改分组顺序，列表里直接换组会丢掉备用分组。',
    smartRoutingEmpty: '尚未添加分组',
    statusLabel: '状态',
    selectStatus: '选择状态',
    saving: '保存中...',
    noKeysYet: '暂无 API 密钥',
    ...
    ccsImport: {
      title: '导入到 CCS',
      description: '选择导入的平台和模型。已按当前分组填好默认值，仍可自行修改。',
      platform: '平台',
      model: '模型',
      modelPlaceholder: '例如 claude-sonnet-4-6',
      modelHint: '默认已按平台填好，列表来自当前密钥的 /v1/models。更换平台会套用该平台的默认模型。',
      modelLoading: '正在用当前密钥拉取可用模型…',
      claude: 'Claude Code',
      ...
      pi: 'Pi',
      deeplinkUnsupported:
        'CC Switch 当前不允许通过深链导入该平台（会报 Invalid provider app type）。Pi 需要在 CC Switch 的 Pi 页面手动添加；Claude Desktop 的供应商导入尚未开放。'
    },
    // 配额和有效期
    quotaLimit: '额度限制',
  ...
  modelPlaza: {
    ...
    catalog: {
      billingNotes: '计费说明',
      pricingNote: '基础价格不含分组倍率。不同分组或上下文档位的价格不同时显示价格区间；实付价格还受专属倍率、分时及高峰规则影响，以分组价格详情为准。',
      filters: '模型筛选',
      category: '模型分类',
      allGroups: '全部分组',
      allRates: '全部倍率',
      ...
      cacheRead: '缓存读取',
      partialPricing: '部分分组未配置定价',
      viewGroups: '查看 {count} 个分组倍率',
      imageRate: '图片独立倍率',
      priceDetails: '价格详情'
    },
    filters: {
  ...
  profile: {
    ...
    balanceNotify: {
      ...
      systemDefault: '系统默认值',
      extraEmails: '通知邮箱',
      extraEmailsHint: '账号绑定邮箱默认可直接接收提醒；额外邮箱仍需添加并验证',
      primaryEmail: '账号绑定邮箱',
