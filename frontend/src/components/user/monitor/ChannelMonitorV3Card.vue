<template>
  <article class="channel-console-card group relative z-0 glass-card flex min-h-[286px] flex-col overflow-visible rounded-[24px] p-5 text-left hover:z-20">
    <header class="channel-console-card__header flex items-start gap-3">
      <span class="channel-console-card__provider grid h-9 w-9 shrink-0 place-items-center rounded-xl ring-1 ring-black/5 dark:ring-white/10" :class="providerGradient(row.platform)">
        <ProviderIcon :provider="row.platform" :size="20" />
      </span>
      <div class="min-w-0 flex-1">
        <div class="channel-console-card__name truncate text-base font-semibold text-gray-900 dark:text-gray-100">{{ groupLabel }}</div>
        <div class="channel-console-card__meta mt-1 flex min-w-0 flex-wrap items-center gap-1.5">
      ...
      <span
            ...
            class="channel-console-card__sample shrink-0 rounded-full bg-gray-100 px-2.5 py-1 text-xs font-semibold text-gray-600 dark:bg-dark-700 dark:text-gray-300"
          >{{ t('channelMonitorV3.unknown') }}</span>
    </header>

    <div class="channel-console-card__metrics mt-5 grid grid-cols-3 gap-2">
      <div class="channel-console-card__metric rounded-2xl border border-slate-200/80 bg-slate-50/85 p-3 dark:border-dark-700/50 dark:bg-dark-900/40">
