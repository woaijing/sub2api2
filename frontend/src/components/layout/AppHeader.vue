<template>
  <header
    class="glass sticky top-0 z-30 border-b border-ui-line"
    :class="{ 'console-header': isConsoleWorkspace }"
  >
    <div class="header-inner flex h-16 items-center justify-between gap-2 px-2 sm:px-4 md:px-6">
      <!-- Left: Mobile Menu Toggle + Page Title -->
      <div class="header-location flex shrink-0 items-center gap-2 sm:gap-4">
        <button
        ...
        <nav v-if="isConsoleWorkspace" class="console-breadcrumb" :aria-label="pageTitle">
          <router-link :to="isAdminConsole ? '/admin/dashboard' : '/dashboard'" class="console-breadcrumb-home" :title="appStore.siteName">
            {{ appStore.siteName }}
          </router-link>
          <Icon name="chevronRight" size="xs" aria-hidden="true" />
          <h1 v-if="isAdminConsole" class="console-admin-title" :title="pageTitle">{{ pageTitle }}</h1>
      ...
      <div class="header-actions flex min-w-0 items-center gap-1 sm:gap-3">
        ...
        <a
          ...
          class="header-shortcut hidden items-center gap-1.5 rounded-lg px-2.5 py-1.5 text-sm font-medium text-gray-600 transition-colors hover:bg-gray-100 hover:text-gray-900 dark:text-dark-400 dark:hover:bg-dark-800 dark:hover:text-white sm:flex"
          :title="t('nav.docs')"
        >
        ...
        <div
          ...
          :tabindex="isConsoleWorkspace ? 0 : undefined"
          :aria-label="isConsoleWorkspace ? `${balanceAvailableText} ${formatHeaderMoney(availableBalance)}` : undefined"
        >
          <svg
          ...
          <span class="header-balance-value text-sm font-semibold text-primary-700 dark:text-primary-300">
            {{ formatHeaderMoney(availableBalance) }}
          </span>
          <span
            v-if="frozenBalance > 0"
            class="header-balance-frozen rounded-full bg-amber-100 px-1.5 py-0.5 text-xs font-medium text-amber-700 dark:bg-amber-900/40 dark:text-amber-200"
          >
            {{ balanceFrozenLabel }}
          </span>
          <div
            class="header-balance-detail pointer-events-none absolute right-0 top-full mt-2 hidden w-56 rounded-lg border border-gray-200 bg-white p-3 text-xs shadow-lg group-hover:block dark:border-dark-700 dark:bg-dark-800"
          >
        ...
        <div v-if="user" class="relative" ref="dropdownRef">
          ...
          >
            <div class="header-avatar flex h-8 w-8 items-center justify-center overflow-hidden rounded-xl bg-gradient-to-br from-primary-500 to-primary-600 text-sm font-medium text-white shadow-sm">
              <img
                v-if="avatarUrl"
                :src="avatarUrl"
                :alt="displayName"
          ...
          <transition name="dropdown">
            <div v-if="dropdownOpen" class="dropdown right-0 mt-2 w-56">
              ...
              <div class="border-t border-gray-100 py-1 dark:border-dark-700">
                <button
                  @click="handleLogout"
                  class="header-logout dropdown-item w-full text-red-600 hover:bg-red-50 dark:text-red-400 dark:hover:bg-red-900/20"
                >
                  <svg
                    class="h-4 w-4"
                    fill="none"
