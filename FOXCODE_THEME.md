...
## 控制台扩展（2026-09-19 / 0.1.331）

控制台样式现扩展到管理页、支付/订单/订阅、个人资料/兑换/返利/白名单和渠道页；
路由清单位于 `useConsoleWorkspace.ts`。`console-account.css` 负责账户类页面布局，
`console-studio.css` 提供紧凑仪表盘、管理工具栏和表面层次，颜色继续复用 `console-shell.css`
与 FoxCode 变量，不能恢复上游蓝色 token。`channel-console.css` 负责渠道页样式。
独立支付结果页直接使用 `--ui-*`，以便无 AppLayout 时仍保持浅深主题和按钮对比度。

仪表盘增加总用量/Token 明细切换，模型环图使用 `chartSeriesColors`，趋势图保留 `useChartTheme`；
平台拆分和最近使用在展开时加载。手机端保留日期输入纵排，分组操作和图表工具栏允许换行。
后台品牌设置仍控制站点名称；公开首页、登录页、模型广场和密钥提供商选择保持定制设计。

预览支持 <http://127.0.0.1:4317/__preview/user> 与 <http://127.0.0.1:4317/__preview/admin>，
提供 15 个新增页面的演示数据；设置页只支持部分演示接口。生产构建保持隔离。
本轮累计完成 93 组页面显示检查，包含桌面、手机、中英文和浅深主题；
更多改动来源、实际测试结果与未验证范围见 `UPSTREAM_UPDATES.md` 的 0.1.331 记录。
